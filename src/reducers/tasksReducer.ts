...
export const tasksReducer = (
	...
	switch (action.type) {
		case 'ADD-TITLE_TASKS': {
			...
			return {...state, [action.payload.dayID]: [newTask, ...state[action.payload.dayID]]}
		}
		case 'REMOVE-TITLE_TASKS': {
			...
			return {
				...state,
				[action.payload.dayID]: state[action.payload.dayID].filter(el => el.id !== action.payload.taskID)
			}
		}
		case 'CHANGE-TITLE_TASKS-STATUS': {
			...
			// }
			return {
				...state, [action.payload.dayID]: state[action.payload.dayID].map(el =>
					el.id === action.payload.taskID
						? {...el, isDone: !action.payload.isDoneTask}
						: el)
			}
		}

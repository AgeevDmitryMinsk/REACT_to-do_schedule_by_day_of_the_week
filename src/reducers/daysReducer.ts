...
export const daysReducer = (state: daysType[], action: daysReducerType): Array<daysType> => {
	switch (action.title) {
		case "ADD-NEW-TODO-LIST": {
			console.log("ADD-NEW-TODO-LIST")
			//setDays([...days, {id: newTodoListID, title: newTodoListTitle, filter: "all"}])
			return [...state, {id: action.payload.newTodoListID, title: action.payload.newTodoListTitle, filter: "all"}]
		}
		...
		case "SORT-TITLES-ON-BUTTON-STATUS": {
			console.log("SORT-TITLES-ON-BUTTON-STATUS")
				...
				return [...state.map(el => el.id === action.payload.dayID ? {...el, filter: action.payload.value} : el)]
		}
		case "REMOVE-DAY": {
		...
		case "CHANGE-WEEKDAY-TITLE": {
			...
			return [...state.map(el=> el.id=== action.payload.dayID ? {...el, title: action.payload.newWeekDayTitle}:el)]
		}
